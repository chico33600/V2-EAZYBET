/*
  # Fix friends leaderboard to include both friendships and referrals

  1. Changes
    - Drop and recreate get_friends_leaderboard
    - Include users with friendships.status='accepted'
    - ALSO include referral relationships (parrain/filleul)
    - Include current user
  
  2. Important
    - Global leaderboard (get_leaderboard) shows ALL users
    - Friends leaderboard shows ONLY: accepted friends + referral connections
    - If no friends/referrals exist, shows only current user
*/

-- Drop the function completely
DROP FUNCTION IF EXISTS get_friends_leaderboard(uuid, integer, integer);

-- Recreate with both friendships and referrals support
