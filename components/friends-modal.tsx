    ...
    try {
      const { data: requests, error } = await supabase
        ...
        .select(`
          ...
          user_id,
          created_at,
          sender:profiles!friendships_user_id_fkey(id, username, diamonds, leaderboard_score)
        `)
      ...
      const formattedRequests = (requests || []).map((req: any) => ({
        ...
        leaderboard_score: req.sender.leaderboard_score,
                ...
                <AnimatePresence>
                  {sentRequests.map((request, index) => (
                    ...
                    >
                      ...
                      <Button
                        onClick={() => cancelSentRequest(request.friendship_id)}
                        size="sm"
                        variant="ghost"
                        className="text-red-400 hover:text-red-300 hover:bg-red-500/10"
                      >
